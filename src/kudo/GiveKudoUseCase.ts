...
export class GiveKudoUseCase {
    ...
    public giveKudo(personTeamsId: string, personName: string, teamId: string): string {
        let record: KudoRecord = this.kudoRecordDb.findRecord(personTeamsId, teamId);
        if (!record.exists()) {
        ...
        return `${record.personName} has *${record.kudos}* point${record.kudos > 1 ? 's' : ''}. ${this.getRandomdEncouragement()}`
    }
