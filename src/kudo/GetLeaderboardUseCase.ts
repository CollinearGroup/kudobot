...
export class GetLeaderboardUseCase {
  ...
  async get(msgContext: TurnContext) {
    ...
    const sortedAllRecords = allRecords.sort((a, b) => a.kudos > b.kudos ? -1 : 1);

    const leaderBoard = sortedAllRecords.map((record, index) => this.recordToRow(record, index + 1)).join('\n');
    return `**${teamName}**\n\n${leaderBoard}`;
  }

  private recordToRow(record: KudoRecord, row: number) {
    return `${row}. ${record.personName} has *${record.kudos}* point${record.kudos > 1 ? 's' : ''}.`
  }
}
