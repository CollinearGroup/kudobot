...
export class KudoRecordFileDbGateway implements KudoRecordDBGateway {
  ...
  constructor() {
    const parsedObjects = JSON.parse(fs.readFileSync(this.localKudoStoreFile).toString())
    this.kudoRecords = parsedObjects.map(object => {
      return new KudoRecord(object.personId, object.personName, object.teamId, object.kudos);
    });
  ...
  public findRecord(personId: string, teamId: string): KudoRecord {
    ...
    return this.kudoRecords.find(record => record.teamId == teamId && record.personId == personId) || new NoopKudoRecord();
  }
